...
*² LSTM: Mạng nhớ dài-ngắn hạn, vượt trội ở tầm dự báo dài (24h) nhờ khả năng lưu trữ temporal patterns.*
*Ghi chú: MASE < 1,0 nghĩa là mô hình tốt hơn Persistence Baseline (Hyndman & Koehler, 2006 [1]).*
...
| 8 | Lee et al. | 2024 | Macau | Stack(LSTM+XGB) | 5,42 | 8,13 | 0,94 | 24h | *Applied Sciences*, DOI: 10.3390/app14125062 |
...
Bảng 4.4: Thống kê phần dư của các mô hình tốt nhất
...
- GRU và LightGBM có độ lệch chuẩn phần dư (6,70 - 7,11 µg/m³) thấp hơn đáng kể so với Persistence (8,62 - 9,07 µg/m³), chứng tỏ khả năng dự báo ổn định hơn.
- LightGBM ở 24h có độ lệch trung bình âm lớn (−3,04), cho thấy mô hình có xu hướng dự báo cao hơn (overestimate) thực tế.
...
|------|------------------|------------------|-------------------|
| 1 | pm25_lag_1h (3,42) | pm25_roll_24h_mean (2,35) | pm25_lag_1h (2,20) |
| 2 | co2 (1,20) | hour_sin (1,90) | hour_cos (0,91) |
| 3 | pm25_roll_24h_mean (0,99) | pm25_roll_24h_min (1,18) | pm25_lag_24h (0,66) |
| 4 | pm25_lag_24h (0,56) | co2_lag_48h (0,59) | diem_suong_lag_1h (0,53) |
| 5 | do_am (0,47) | pm25_roll_6h_min (0,51) | diem_suong_lag_24h (0,39) |

**Nhận định:**
- Tại h=1: Đặc trưng `pm25_lag_1h` chiếm uy thế hoàn toàn (SHAP = 3,42), xác nhận rằng autocorrelation quyết định tất cả ở khung ngắn — không mô hình ML nào thắng được Persistence.
- Tại h=6: Trọng tâm dịch chuyển sang các đặc trưng cuộn trung bình 24h (`pm25_roll_24h_mean`) và chu kỳ ngày đêm (`hour_sin`). Điều này giải thích vì sao ML (LightGBM) chiếm ưu thế: nó biết tổng hợp các xu hướng dài hạn.
- Tại h=24: Cả lag_1h và lag_24h đều xuất hiện, cùng với các yếu tố môi trường như điểm sương (`diem_suong`). Mô hình cần học cả chu kỳ ngày (`hour_cos`) lẫn tín hiệu vật lý để dự báo xa.
...
|-------------|---------|-------------|-------------|--------------|-------------------|----------|
| **Conformal Prediction** | LightGBM | 81,0% | 76,2% | 78,5% | 10,2 - 10,4 | Gần target nhất |
| **Quantile Regression** | LightGBM | 86,1% | 79,3% | 83,7% | 7,6 - 9,8 | Hẹp nhất, coverage tốt |
...
| LSTM_v9_expert_30m | Expert | 1,064 | 0,558 | 0,616 |
...
1. **Dữ liệu đơn cảm biến**: Chỉ sử dụng 1 cảm biến IoT duy nhất, không có thông tin không gian (spatial). Kết quả chưa được xác nhận trên nhiều địa điểm.
...
3. **R² thấp**: R² = 0,37 (h=1) thấp hơn nhiều so với các công trình khác (0,70 - 0,96), duy phản ánh đúng thực tế của dữ liệu IoT đơn cảm biến với pipeline anti-leakage nghiêm ngặt.
...
7. **Tối ưu Ensemble**: Mặc dù LSTM đạt MASE = 0,676 tại h=24 và Ensemble_Stack = 0,696, nhưng ở h=1, việc stacking các mô hình học máy chưa vượt qua được TFT (0,987). Điều này cho thấy với dataset nhỏ, meta-learner cần được thiết kế cẩn trọng hơn để tránh overfitting, và cơ chế attention có thể là hướng đi tốt hơn cho short-horizon.
