...
## Phiên 2026-04-18 (Sáng) - Hoàn thiện Scientific Observable UI
**Mục tiêu:**
- Hoàn thiện 100% giao diện Scientific Observability cho toàn bộ các trang (Overview, EDA, Feature Engineering, Pipeline, Forecast, DL, Ensemble).
- Tùy chỉnh tương phản CSS cho Dark Mode & Light Mode (tắt các config cứng màu lỗi).
- Các thẻ KPI và Project Stats, Pipeline Block đã được hardcode dark background (`#1A1F2E`) cùng màu chữ sáng (`#F8FAFC`, `#E2E8F0`) để giữ vẻ ngoài cao cấp và không bị lỗi hiển thị Light Mode.
**Trạng thái kế tiếp:**
- UI/UX đã hoàn hảo.
- Chiều nay (2026-04-18): Build Docker image, push lên Docker Hub, và chạy thử trực tiếp container trên Docker Desktop cho Hội đồng.

## [2026-05-05] Phase 1-3 EDA Audit & Enhancement
- **D2 (Mutual Information)**: Th�m MI Heatmap d? b?t c�c tuong quan phi tuy?n. Ph�t hi?n Nhi?t d? v� �i?m suong d?n d?t PM2.5.
- **A3 (Conditional Distribution)**: B? sung Violin plot theo kho?ng nhi?t d?, minh ch?ng hi?n tu?ng ngh?ch nhi?t (long-tail pollution) ? nhi?t d? th?p (<26�C).
- **B3 (Weekday/Weekend)**: Boxplot so s�nh ng�y thu?ng vs cu?i tu?n, cho th?y ph�t th?i T7/CN kh�ng gi?m nhi?u do ngu?n h?n h?p.
- **C2 & C3 (Missing & Imputation)**: Tr?c quan h�a gap pattern v� ch?ng minh hi?u qu? di?n khuy?t (Spline+KNN) so v?i data g?c.
- **Granger Causality Fix**: C?p nh?t insight card d? gi?i th�ch l?i log scale l�m ?n c�c bi?n c� p-value=0.0.
