...
v9 update: Column naming changed from `_Xh` (hours) to `_Xs` (steps)
to support multi-resolution pipelines (15min, 30min, 1h).
"""
...
@pytest.fixture()
def segmented_df(sample_df):
    """Create a sample DataFrame with segment IDs for testing segment-aware features."""
    df = sample_df.copy()
    # Create 2 segments: rows 0-99 = seg 1, rows 100-199 = seg 2
    df["segment_id"] = [1] * 100 + [2] * 100
    return df
...
class TestLagFeatures:
    def test_creates_correct_columns(self, sample_df):
        ...
        assert "pm25_lag_1s" in result.columns
        assert "pm25_lag_2s" in result.columns
        assert "pm25_lag_3s" in result.columns

    def test_lag_values_are_correct(self, sample_df):
        result = create_lag_features(sample_df, lags=[1], include_features=False)
        # lag_1s[t] should equal pm25[t-1]
        assert result["pm25_lag_1s"].iloc[1] == result["pm25"].iloc[0]
        assert result["pm25_lag_1s"].iloc[5] == result["pm25"].iloc[4]

    def test_lag_1_is_nan_at_first_row(self, sample_df):
        result = create_lag_features(sample_df, lags=[1], include_features=False)
        assert pd.isna(result["pm25_lag_1s"].iloc[0])
    ...
    def test_no_future_leakage(self, sample_df):
        ...
        # At time t, lag_1s should be value at t-1
        for i in range(1, 10):
            assert result["pm25_lag_1s"].iloc[i] == sample_df["pm25"].iloc[i - 1]

    def test_include_feature_lags(self, sample_df):
        result = create_lag_features(sample_df, lags=[1], include_features=True)
        assert "nhiet_do_lag_1s" in result.columns
        assert "co2_lag_1s" in result.columns
    ...
    def test_default_lags(self, sample_df):
        ...
        assert "pm25_lag_168s" in result.columns
    ...
    def test_segment_aware_lag(self, segmented_df):
        ...
        assert result["pm25_lag_1s"].iloc[101] == segmented_df["pm25"].iloc[100]
...
class TestRollingFeatures:
    ...
    def test_rolling_std_creates_nan_for_single_value(self, sample_df):
        ...
        assert pd.isna(result["pm25_roll_3s_std"].iloc[0])

    def test_default_funcs(self, sample_df):
        result = create_rolling_features(sample_df, windows=[3])
        assert "pm25_roll_3s_mean" in result.columns
        assert "pm25_roll_3s_std" in result.columns
        assert "pm25_roll_3s_min" in result.columns
        assert "pm25_roll_3s_max" in result.columns
...
class TestEWMFeatures:
    def test_creates_correct_columns(self, sample_df):
        ...
        assert "pm25_ewm_12s_mean" in result.columns
        assert "pm25_ewm_12s_std" in result.columns
    ...
    def test_ewm_is_shifted(self, sample_df):
        ...
        assert not pd.isna(result["pm25_ewm_12s_mean"].iloc[1])
...
class TestDiffFeatures:
    def test_creates_diff_columns(self, sample_df):
        ...
        assert "pm25_diff_1s" in result.columns
        assert "pm25_diff_24s" in result.columns
        assert "pm25_pct_change_1s" in result.columns
        assert "pm25_pct_change_24s" in result.columns

    def test_diff_value_correct(self, sample_df):
        """Diff uses shift(1) → diff_1s[t] = y[t-1] - y[t-2] (anti-leakage).
        ...
        assert pd.isna(result["pm25_diff_1s"].iloc[0])
        assert pd.isna(result["pm25_diff_1s"].iloc[1])
        # iloc[2] = y[1] - y[0] (uses only past values, no y[2])
        expected_diff = sample_df["pm25"].iloc[1] - sample_df["pm25"].iloc[0]
        np.testing.assert_almost_equal(result["pm25_diff_1s"].iloc[2], expected_diff)

    def test_segment_aware_diff(self, segmented_df):
        """Diff should not cross segment boundaries."""
        result = create_diff_features(segmented_df, segment_col="segment_id")
        # First row of segment 2: shifted within segment → NaN
        assert pd.isna(result["pm25_diff_1s"].iloc[100])
        # Second row of segment 2: shifted within segment → still NaN (shift+diff warmup)
        assert pd.isna(result["pm25_diff_1s"].iloc[101])
...
class TestBuilder:
    ...
    def test_no_data_leakage_in_built_features(self, sample_df):
        ...
        for col in lag_cols[:3]:  # Check first 3 lag features
            lag_steps = int(col.split("_lag_")[1].replace("s", ""))
            base_col = col.split("_lag_")[0]
            for i in range(lag_steps, min(lag_steps + 5, len(result))):
                expected = sample_df[base_col].iloc[i - lag_steps]
                actual = result[col].iloc[i]
                np.testing.assert_almost_equal(actual, expected, decimal=10)

    def test_segment_aware_build(self, segmented_df):
        """Full pipeline should work with segment_col."""
        result = build_features(segmented_df, drop_na=True, segment_col="segment_id")
        assert len(result.columns) > 50
        assert result.isna().sum().sum() == 0
