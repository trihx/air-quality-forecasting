...
def main():
    ...
    for horizon in [1, 6, 24]:
        ...
        test_real_mask = df["is_imputed"].iloc[val_end:] == False if "is_imputed" in df.columns else [True] * len(y_test)  # noqa
        ...
        print(f"  Persistence: MAE={persist_mae:.3f}, RMSE={persist_rmse:.3f}, R²={persist_r2:.4f}, MAPE={persist_mape:.2f}%")
        ...
        try:
            ...
            print(f"  LightGBM:    MAE={lgb_mae:.3f}, RMSE={lgb_rmse:.3f}, R²={lgb_r2:.4f}, MAPE={lgb_mape:.2f}%, MASE={lgb_mase:.3f}")
        ...
        try:
            ...
            print(f"  GRU:         MAE={gru_mae:.3f}, RMSE={gru_rmse:.3f}, R²={gru_r2:.4f}, MAPE={gru_mape:.2f}%, MASE={gru_mase:.3f}")
        except Exception as e:
    ...
    for h, models in results.items():
        for model, m in models.items():
            print(f"  {model:<20} {h:<8} {m['MAE']:>8.3f} {m['RMSE']:>8.3f} {m['R2']:>8.4f} {m.get('MAPE','N/A'):>8} {m['MASE']:>8.3f}")
        print()
