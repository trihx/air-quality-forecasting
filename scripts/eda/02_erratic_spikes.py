...
def plot_distribution(df: pd.DataFrame):
    ...
    ax.text(0.5, 0.4, text_content, transform=ax.transAxes, fontsize=12,
...
def plot_spikes_timeline(df: pd.DataFrame):
    ...
    ax.annotate('Đỉnh dị thường\n(Kẹt xe / Đốt cháy?)', xy=(max_idx, max_val), xytext=(max_idx, max_val + 20),
                arrowprops=dict(facecolor=ERROR_COLOR, shrink=0.05),
                horizontalalignment='center', color=ERROR_COLOR, fontweight='bold', zorder=3)
    
    ax.set_title(f"Sự biến động khó lường của PM2.5 trong khoảng thời gian {worst_month.strftime('%m/%Y')}", 
