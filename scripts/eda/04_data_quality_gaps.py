...
def plot_missing_barcode(df: pd.DataFrame):
    ...
    ax.text(0.01, 0.5, f"Tổng số dữ liệu bị thiếu: {missing_pct:.1f}%", 
...
def plot_recovery_bar(report_data: dict):
    ...
    if hours_recoverable == 0: return # Fallback guard
    ...
    for bar in bars:
        ...
        ax.text(bar.get_x() + bar.get_width()/2, yval + 20, 
