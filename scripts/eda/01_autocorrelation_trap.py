...
def plot_autocorrelation_story(df: pd.DataFrame):
    ...
    plot_acf(
        df["pm25"], lags=72, ax=ax1, color=PRIMARY_COLOR, alpha=0.05, title="Tự tương quan (ACF) - Lags up to 72 hours"
    )
...
def plot_horizon_scatter(df: pd.DataFrame):
    ...
    for ax_idx, col, title, color in settings:
        ...
        ax.hexbin(df_lag[col], df_lag["y_t"], gridsize=40, cmap="Blues", bins="log", mincnt=1)
