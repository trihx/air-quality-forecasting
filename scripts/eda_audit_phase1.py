...
def generate_stl_decomposition(df: pd.DataFrame) -> dict:
    ...
    axes[1].text(0.98, 0.85, strength_text, transform=axes[1].transAxes, ha="right",
                fontsize=9, bbox=dict(boxstyle="round,pad=0.3", facecolor="#2D3139", alpha=0.8))

    # Panel 3: Seasonal
    axes[2].plot(seasonal.index, seasonal.values, color=ACCENT_GREEN, linewidth=0.8)
    axes[2].set_ylabel("Seasonal")
    axes[2].axhline(y=0, color="#555", linestyle="--", linewidth=0.5)
    ...
    axes[3].axhline(y=0, color="#555", linestyle="--", linewidth=0.5)
    ...
    # Mark day boundaries
    for d in range(1, 8):
        ax.axvline(x=d * 24, color="#555", linestyle=":", linewidth=0.5)
...
def generate_periodogram(df: pd.DataFrame) -> dict:
    ...
    for period, label, color in [
        ...
        axes[1].text(period * 1.1, max(psd[mask]) * 0.3, label, color=color, fontsize=8, rotation=90, va="top")
