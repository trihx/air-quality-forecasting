"""v9 Phase 5B-1h — TFT 1h Pipeline (OOM Fix).

Uses CPU fallback, base features, and batch_size=8 to prevent MPS OOM errors
that occurred during previous 1h retraining attempts.
...
# FORCED CPU FALLBACK to prevent OOM
...
def run_pipeline(freq: str, df_base: pd.DataFrame):
    print(f"\n{'=' * 70}", flush=True)
    print(f"[TFT-1H] Temporal Fusion Transformer - {freq} Dataset", flush=True)
    ...
    print(f"  Dataset rows: {len(df_dl):,}", flush=True)
    ...
    for h_hours in HORIZONS_HOURS:
        ...
        train_loader = DataLoader(train_ds, batch_size=8, shuffle=True)  # BATCH SIZE REDUCED to 8
    ...
    _save(results, "tft_1h_metrics", freq)
    _save(preds, "tft_1h_preds", freq)
    print(f"\n[v9] ✅ TFT 1h ({freq}) saved.", flush=True)


def main():
    print("=" * 70, flush=True)
    print(f"v9 TFT 1h RETRAIN — OOM Fix", flush=True)
    ...
    path_1h = data_dir / "marts_features.csv"
    if path_1h.exists():
        df_1h = pd.read_csv(path_1h, index_col=0, parse_dates=True)
        run_pipeline("1h", df_1h)
    else:
        print(f"⚠️ {path_1h} not found. Run v9_rebuild_data.py first.")

    print(f"\n✅ Total TFT 1h time: {time.time() - t0:.1f}s", flush=True)
